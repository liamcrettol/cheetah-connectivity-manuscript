...
\TODO{Sentence above now matches what was actually run. Two things still open:
1. The protocol still says RES-01 to RES-06. Add an amendment-log entry in protocol
   section 19 recording that the implemented set differs, so the divergence is on the
   record rather than just quietly resolved here.
2. Cites worth adding in this paragraph: \textcite{zeller2018equal} alongside
...
\TODO{The clause above is restored verbatim from the locked protocol, so H4 is now
falsifiable as written, like H1 to H3. The harder problem is that it cannot be
evaluated: the weight sets W1--W3 and the cell-level index were never built, so there
is nothing to test stability across.
TWO HONEST ROUTES, and this one is yours to pick because it is a predeclaration
question, not an editing one:
  (a) Leave H4 exactly as locked and report in Results that it could not be evaluated
      because the index was not built. Costs you a result, keeps the predeclaration
      clean. This is the safer route.
  (b) Amend H4 to the link-level robust / uncertain split, which the data can answer.
      If you do this, it MUST go in the protocol amendment log with a date, and both
      the original and amended wording must appear. Rewriting a hypothesis after seeing
      the results and not saying so is the exact failure the predeclaration exists to
      prevent.
Do not simply delete H4.
